export default function HeroSection() {
  return (
    <section 
      className="min-h-screen px-4 sm:px-6 lg:px-8 pt-32 pb-20 relative flex items-center justify-center overflow-hidden"
    >
      {/* Background image - only top half of hero */}
      <div
        className="absolute top-0 left-0 right-0 h-[70%] bg-cover bg-center bg-no-repeat"
        style={{
      ...
      <div 
        className="absolute top-0 left-0 right-0 h-[70%] bg-cover bg-center bg-no-repeat opacity-30"
        style={{
          backgroundImage: 'url(/images/dots.png)',
        }}
      ></div>
      {/* Dark overlay to darken the image (top half only) */}
      <div className="absolute top-0 left-0 right-0 h-[70%] bg-black/20"></div>
      {/* Gradient fade from images into white at the end of hero images */}
      <div className="absolute top-0 left-0 right-0 h-[70%] bg-gradient-to-b from-transparent via-transparent to-white"></div>
      <div className="max-w-4xl mx-auto text-center relative z-10">
        ...
        <h1 className="text-4xl sm:text-5xl md:text-6xl font-semibold tracking-tight mb-6 text-slate-900">
          <span className="block font-light text-slate-800/90 text-[1.18em] sm:text-[1.22em]">
            Build Second Income With
          </span>
          <span className="block font-semibold bg-clip-text text-transparent bg-gradient-to-r from-[rgb(168,110,56)] via-amber-400 to-[rgb(168,110,56)] text-[1.18em] sm:text-[1.22em]">
            Backtested Trading Rules
          </span>
        </h1>
        <p className="text-base sm:text-lg md:text-xl text-slate-800/90 leading-relaxed mb-8 max-w-2xl mx-auto">
